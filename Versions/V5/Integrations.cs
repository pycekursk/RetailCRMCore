...
namespace RetailCRMCore.Versions.V5
{
  public partial class Client
  {
    ...
    /// </summary>
    /// <param name="code">что вам падлы мешало описать что такое CODE????</param>
    /// <returns></returns>
    ...
    public Response IntegrationsSettingsEdit(Dictionary<string, object> integrationModule)
    {
      ...
      return Request.MakeRequest(
          ...
          {
                    { "integrationModule", Newtonsoft.Json.JsonConvert.SerializeObject(integrationModule) }
          }
      );
    }

    public Response IntegrationsSettingsEdit(IntegrationModule integrationModule)
    {
      return Request.MakeRequest(
